...
```bash
mise install          # Install tool versions (Bun, Node)
...
```bash
bun run --filter slack dev     # Run Slack plugin in dev mode
```
...
bun test --filter plugins/slack # Run tests for specific plugin
```
...
- Test complete message flows (inbound and outbound)
- Verify access control (pairing, allowlist, gating)
- Test message chunking and file attachments
